const projects = [
  ...
  {id:'draconic',category:'Brand',name:'DRACONIC',type:'Brand Building',idea:'A Voice for AI-led Trading',preview:'assets/cases/draconic/all/DVdw8fsiogc.jpg',previewVideo:'assets/cases/draconic/all/DVdw8fsiogc.mp4',roles:['Positioning','Tone of Voice','Social Strategy','Creative Direction'],brief:'Define how an AI-powered trading product should position itself and communicate on social media.',response:'The positioning and social system gave the product a clear point of view around the judgement, pressure and discipline of trading.',contribution:'Led the creative planning, positioning, tone of voice and social strategy. I set the direction for the visual and verbal system, working with art directors and video editors to produce the final content.',videos:['assets/cases/draconic/all/DVdw8fsiogc.mp4','assets/cases/draconic/all/DVGgjFACvMV.mp4','assets/cases/draconic/all/DVLbOSgigir.mp4','assets/cases/draconic/all/DVQnJqUCv7F.mp4'],items:[['DUimAYhkW22','local'],['DU-3iuKkWnp','local'],['DU5rh2MEthR','local']]},
...
const campaignCases = [
  ...
  },
  {
    ...
    hero:'assets/campaigns/duolingo-vaathi/hero.jpg', film:'https://www.youtube.com/embed/CSDs8OtyI8A',
    ...
    links:[['CAMPAIGN COVERAGE','https://campaignbriefasia.com/2025/01/14/talented-india-infuses-tamil-pop-culture-into-duolingos-learn-english-from-tamil-campaign/'],['WATCH ON YOUTUBE','https://www.youtube.com/shorts/CSDs8OtyI8A'],['WATCH ON INSTAGRAM','https://www.instagram.com/p/DEohYRSShYV/']]
  }
];

function openCampaign(campaign) {
  const links=campaign.links.map(([label,url])=>`<a href="${url}" target="_blank" rel="noreferrer">${label} <svg class="arr-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 17 17 7M8 7h9v9"/></svg></a>`).join('');
...
function openProject(project) {
  ...
  const sourceLinks = project.links ? `<nav class="campaign-case-links project-links" aria-label="Project links">${project.links.map(([label,url])=>`<a href="${url}" target="_blank" rel="noreferrer">${label} <svg class="arr-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 17 17 7M8 7h9v9"/></svg></a>`).join('')}</nav>` : '';
